...
class MQTTCog(commands.Cog):
    ...
    def __init__(self, bot: Bot, from_env: bool = True, creds: dict | None = None):
        ...
        self.subscribe_handlers = [
            ...
            MQTTTopicHandler(f"{self.MQTT_ROOT}/subscribe/message", self.add_message_subscription),
    ...
    def create_mqtt_client(self) -> aiomqtt.Client:
        if self.creds and self.creds.get("from_env"):
            ...
            raise ValueError("No MQTT credentials provided")
    ...
    async def mqtt_loop(self):
        ...
        while True:
            try:
                ...
                self.logger.error(f"Unexpected error in MQTT loop: {e} - reconnecting in 5 seconds...")
                unknown_exceptions += 1
                await asyncio.sleep(5)
                if unknown_exceptions > 5:
                    self.logger.critical("Too many unknown exceptions in MQTT loop, stopping.")
                    break
        ...
        try:
            ...
            if not name:
                await self.publish_message(
                    ...
                    {"msg": "Subscription must have a name", "status": "error", "name": None, "error": "missing_name"},
                )
            ...
            if not author_id and not channel_id and not guild_id:
                self.logger.error("At least one of author_id, channel_id, or guild_id must be provided")
                await self.publish_message(
                    f"{self.MQTT_ROOT}/subscribe/message/nack",
                    {"msg": "At least one of author_id, channel_id, or guild_id must be provided", "status": "error", "name": name, "error": "invalid_parameters"},
                )
                return
            ...
            await self.publish_message(
                f"{self.MQTT_ROOT}/subscribe/message/nack",
                {"msg": "Invalid JSON payload", "status": "error", "name": None, "error": "invalid_json"},
            )
        except Exception as e:
    ...
    async def send_message(self, payload: str):
        ...
        try:
            ...
            if not channel_id_raw or not (content or embeds or typing):
                self.logger.error("send_message requires channel_id and at least one of content, embeds, or typing")
                await self.publish_message(
                    f"{self.MQTT_ROOT}/send_message/nack",
                    {"msg": "channel_id and at least one of content, embeds, or typing required", "status": "error", "channel_id": None},
                )
                return
            ...
            if not channel or not isinstance(channel, discord.TextChannel):
                self.logger.error(f"Channel {channel_id} not found or is not a text channel")
                return
            ...
            await self.publish_message(
                f"{self.MQTT_ROOT}/send_message/nack",
                {"msg": str(e), "status": "error", "channel_id": str(channel_id) if 'channel_id' in locals() else None},
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        for sub in self.subscribed_messages:
            if (
                ...
                and (not sub.guild_id or (message.guild and message.guild.id == sub.guild_id))
            ):
                self.logger.info(f"MQTT: Message from {message.author} in {message.channel} matches subscription {sub.name}")
                # This subscription matches the message
                await self.publish_message(f"{self.MQTT_ROOT}/events/on_message/{sub.name}", payload)
                
    @commands.Cog.listener()
    async def on_ready(self):
        self.logger.info(f"MQTT Cog connected as {self.bot.user} (ID: {self.bot.user.id})")
        if not self.mqtt_client:
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.status != after.status:
            ...
            await self.publish_message(f"{self.MQTT_ROOT}/events/on_presence_update", payload)
            self.logger.info(f"Published presence update for {after} from {before.status} to {after.status}")