...
class ChannelConfigEditor:
    ...
    async def start(self):
        ...
        providers = self.conversation_manager._provider_manager.providers
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        current_channel_config = await self.conversation_manager.get_channel(
        ...
        if (
            ...
            and current_channel_config.provider_name != selected_provider_name
        ):
            hints.append(
                f"Provider changed to {selected_provider_name}. Make sure to update model settings for this new provider!"
            )
        if override_system_prompt:
            hints.append(
                "System prompt will be overridden. Only the channel-specific prompt will be used."
            )
        if channel_prompt and len(channel_prompt) > 500:
            hints.append(
                "Channel prompt is quite long. Consider shortening it for better performance and response times."
            )
        ...
        try:
            ...
            await send_ephemeral_response(
                interaction,
                error="An error occurred while saving the channel configuration. Please try again later.",
            )
            self.conversation_manager._logger.error(
                f"Error saving channel configuration for channel {interaction.channel_id}: {e}"
            )
            return

        await send_ephemeral_response(
            interaction,
            success="Channel configuration saved successfully!"
