...
from ..models import OutputContext, Channel, LLMProfile, GuildRecord
...
class SessionContext:
    ...
    def set_active_profile(self, profile: LLMProfile) -> None:
        ...
        if profile not in self.llm_profiles:
            raise ValueError(f"Profile {profile.name} is not in the list of available profiles")
    ...
    def provider(self) -> Provider:
        ...
        if not provider:
            raise ValueError(f"No provider found for profile {self.llm_profile.name}")
        return provider
