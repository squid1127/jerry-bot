...
from ..models.exceptions import FatalError
...
class MessageQueue:
    ...
    async def _process_message(self, message: Message, attempt: int | str = "unknown") -> Exception | None:
    ...
    async def _process_with_retries(self, message: Message):
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            ...
            if error is None or isinstance(error, FatalError):
                break
        else:
            self._logger.error(
                f"Failed to process message after {self.MAX_RETRIES} attempts: {message}"
            )
            if error:
