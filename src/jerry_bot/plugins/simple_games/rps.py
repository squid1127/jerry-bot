...
class RPSGame(discord.ui.LayoutView):
    ...
    def generate_container(self) -> discord.ui.Container:
        ...
        if self.state == GameState.PICK:
            ...
            choices = discord.ui.ActionRow(
                *(ChoiceButton(choice, self) for choice in Choice)
            )
            ...
            if not winners or len(winners) == len(players):
                ...
                for player in players:
                    result_text += (
                        f"{player.display_name} - {self.choices[player].value}\n"
                    )
            else:
                # Show winners and losers
                for winner in winners:
                    result_text += (
                        f"🏆 **{winner.display_name}** - {self.choices[winner].value}\n"
                    )
